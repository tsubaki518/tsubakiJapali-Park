...
void Sky::Draw() {
	...
	const D3DXVECTOR3 SKY_POSITION = D3DXVECTOR3(0, 0, -100);
	Cube sky[PLANE_NUM];

	//スタート地点から見て左右前後に壁を設置して空を作る
	//後ろの壁
	sky[0].scale = D3DXVECTOR3(PLANE_SIZE, PLANE_SIZE, 50);
	sky[0].position = D3DXVECTOR3(0, 0, 0);
	sky[0].rotation = D3DXVECTOR3(0.1f, 3.141592f, 0);

	//右
	sky[1].scale = D3DXVECTOR3(0, PLANE_SIZE, PLANE_SIZE);
	sky[1].position = D3DXVECTOR3(PLANE_SIZE/2-20, 0, PLANE_SIZE/2);
	sky[1].rotation = D3DXVECTOR3(1.507f +3.1415f, 0, 0);
